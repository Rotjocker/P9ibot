...
async def join_channel(event):
    try:
        await sython1(JoinChannelRequest("@vlod2"))
    ...
    if sender.id == ownerhson_id:
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await sython1.send_message(event.chat_id, f"تم الانتهاء من التجميع | p9")
        ...
        await sython1.send_message(event.chat_id, "تم الانتهاء من التجميع | p9")
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id:
        ...
        joinu = await sython1(JoinChannelRequest('vlod2'))
        channel_entity = await sython1.get_entity('@DamKombot')
        await sython1.send_message('@DamKombot', '/start')
        await asyncio.sleep(4)
        msg0 = await sython1.get_messages('@DamKombot', limit=1)
